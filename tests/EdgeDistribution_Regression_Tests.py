...
class TestEdgeDistributionRegression(unittest.TestCase):
    ...
    def test_samples_regression(self) -> None:
    ...
    def test_mgf_regression(self) -> None:
        ts = [0, 0.1, 1.0]
        for dist_key, ed in self.test_edges.items():
    ...
    def test_mgf_derivative_regression(self) -> None:
        ts = [0, 0.1, 1.0]
        for dist_key, ed in self.test_edges.items():
            ...
            for t, expected in zip(ts, expected_vals):
                ...
                self.assertAlmostEqual(
                    expected, actual, places=5, msg=f"MGF derivative mismatch for {dist_key} at t={t}"
                )

    def test_mgf_derivative2_regression(self) -> None:
        ts = [0, 0.1, 1.0]
        for dist_key, ed in self.test_edges.items():
            ...
            for t, expected in zip(ts, expected_vals):
                ...
                self.assertAlmostEqual(
                    expected, actual, places=5, msg=f"MGF 2nd derivative mismatch for {dist_key} at t={t}"
                )
